...
class FrontEndController extends Controller
{
    ...
    {
        $orders = DB::select("SELECT foodplans.plan_name,users.id as userid,users.ul_name,users.name,users.email,users.phone,orders.* FROM orders JOIN foodplans ON orders.food_id=foodplans.id JOIN users ON orders.users_id=users.id WHERE orders.product_type='plan'");
        return view('admin.frontendsetting.order.planorderlist', compact('orders'));
    }

    public function planorderdays($uid, $oid, $pid)
    {
        $orderdays = DB::select("SELECT orders.id, orderplans.day_id, orderplans.food_type_id, orderplans.order_status, orderplans.order_id, orderplans.plan_id, orderplans.food_item_id, food_items.food_name FROM orders LEFT JOIN orderplans ON orders.food_id=orderplans.plan_id LEFT JOIN food_items ON orderplans.food_item_id=food_items.id WHERE orders.id=$oid ORDER BY orderplans.day_id ASC");
