...
@section('body')
    <div class="container-fluid">
                        ...
                        <table class="table table-hover mb-0">
                            ...
                            @foreach($orderdays as $key=>$orderday)
                                <tr>
                                    ...
                                    <td>{{date('d-m-Y',strtotime($orderday->order_date))}}</td>
                                    <td>
      ...
      <option  value="0" data-id="{{$order->id}}" @if ($order->status==0) selected='selected' @endif >Pending</option>                                       
                                    ...
                                    <td>
                                        <select class="form-control status">
                                            <option  value="0" data-id="{{$orderday->order_id}}" person-id="{{$uid}}" plan-id="{{$orderday->plan_id}}" day-id="{{$orderday->day_id}}" fooditem-id="{{$orderday->food_item_id}}" foodtype-id="{{$orderday->food_type_id}}" @if ($orderday->order_status==0) selected='selected' @endif >Pending</option>                                       
                                            <option value="1" data-id="{{$orderday->order_id}}" person-id="{{$uid}}" plan-id="{{$orderday->plan_id}}" day-id="{{$orderday->day_id}}" fooditem-id="{{$orderday->food_item_id}}" foodtype-id="{{$orderday->food_type_id}}" @if ($orderday->order_status==1) selected='selected' @endif >Confirm</option>

                                            <option value="2" data-id="{{$orderday->order_id}}" person-id="{{$uid}}" plan-id="{{$orderday->plan_id}}" day-id="{{$orderday->day_id}}" fooditem-id="{{$orderday->food_item_id}}" foodtype-id="{{$orderday->food_type_id}}" @if ($orderday->order_status==2) selected='selected' @endif >Preparing</option>

                                            <option value="3" data-id="{{$orderday->order_id}}" person-id="{{$uid}}" plan-id="{{$orderday->plan_id}}" day-id="{{$orderday->day_id}}" fooditem-id="{{$orderday->food_item_id}}" foodtype-id="{{$orderday->food_type_id}}" @if ($orderday->order_status==3) selected='selected' @endif >Delivered</option>
                                        </select>
                                    </td>
...
@section('scripts')
    ...
    $(document).ready(function(){ 
        $('select.status').on('change',function(){
            ...
            var  dayId=$(this).find(":selected").attr("day-id");
            var  fooditemId=$(this).find(":selected").attr("fooditem-id");
            var  foodtypeId=$(this).find(":selected").attr("foodtype-id");

            $.ajax({

                url:'{{url('admin/statusUpdateplanorder')}}',
               // data:'rowID='+ rowID + '&status='+Status,
                data:{orderid:orderId,personid:personId,planid:planId,dayid:dayId,fooditemid:fooditemId,foodtypeid:foodtypeId,status:Status},
                type:'GET',
                 dataType: "json",
